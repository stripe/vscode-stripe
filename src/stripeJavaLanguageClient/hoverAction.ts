...
class JavaHoverProvider implements HoverProvider {
    ...
    async provideHover(
        ...
        if (hoverResponse.contents && hoverResponse.contents[0].value.includes('com.stripe.model')) {
            const stripeMethod = hoverResponse.contents[0].value;
            const resourceAndMethod = stripeMethod.split(' ', 2);
            const apiPath = resourceAndMethod[1].split('.');
            const method = apiPath.slice(-1)[0].split('(')[0];
            const sliced = apiPath.slice(3, -1);
            let url = 'https://stripe.com/docs/api';
            sliced.forEach((x) => (url = url + '/' + x.toLowerCase()));
            url = url + '/' + method;
            return new Hover([new MarkdownString('[Stripe API Reference](' + url + ')')], null);
        }
