...
export class StandardLanguageClient {
  ...
  public async initialize(
    ...
    jdtEventEmitter: EventEmitter<Uri>,
  ): Promise<void> {
    if (this.status !== ClientStatus.Uninitialized) {
      return;
    }

    const hasImported: boolean = await fse.pathExists(
    ...
    context.subscriptions.push(
      commands.registerCommand(Commands.RUNTIME_VALIDATION_OPEN, () => {
      ...
      this.languageClient.onRequest(ConfigurationRequest.type, (params: ConfigurationParams) => {
        ...
        for (const item of params.items) {
          ...
          if (scopeUri && scopeUri.toString() === activeEditor?.document.uri.toString()) {
            ...
            result.push(
              workspace.getConfiguration(null, scopeUri).get(item.section, null /*defaultValue*/),
            );
          }
        }
        return result;
  ...
  private registerCommandsForStandardServer(
    context: ExtensionContext,
    jdtEventEmitter: EventEmitter<Uri>,
  ): void {
    context.subscriptions.push(
      commands.registerCommand(Commands.IMPORT_PROJECTS, async () => {
    ...
    context.subscriptions.push(
      commands.registerCommand(Commands.OPEN_OUTPUT, () =>
        this.languageClient.outputChannel.show(ViewColumn.Three),
      ),
    );
    context.subscriptions.push(
      commands.registerCommand(Commands.SHOW_SERVER_TASK_STATUS, () =>
        serverTaskPresenter.presentServerTaskView(),
      ),
    );

    this.languageClient.onReady().then(() => {
      context.subscriptions.push(
        commands.registerCommand(GRADLE_CHECKSUM, (wrapper: string, sha256: string) => {
      ...
      context.subscriptions.push(
        commands.registerCommand(Commands.CONFIGURATION_UPDATE, (uri) =>
          projectConfigurationUpdate(this.languageClient, uri),
        ),
      );

      context.subscriptions.push(
        commands.registerCommand(Commands.IGNORE_INCOMPLETE_CLASSPATH, () =>
          setIncompleteClasspathSeverity('ignore'),
        ),
      );

      context.subscriptions.push(
        commands.registerCommand(Commands.IGNORE_INCOMPLETE_CLASSPATH_HELP, () => {
          commands.executeCommand(
      ...
      context.subscriptions.push(
        commands.registerCommand(Commands.PROJECT_CONFIGURATION_STATUS, (uri, status) =>
          setProjectConfigurationUpdate(this.languageClient, uri, status),
        ),
      );
          ...
          async (location: LinkLocation | Uri) => {
            ...
            if (location instanceof Uri) {
              ...
              const params: TextDocumentPositionParams = {
                ...
                position: this.languageClient.code2ProtocolConverter.asPosition(
      ...
      context.subscriptions.push(
        commands.registerCommand(Commands.CHANGE_BASE_TYPE, async (item: TypeHierarchyItem) => {
      ...
      context.subscriptions.push(
        commands.registerCommand(
          ...
          (isFullCompile: boolean, token?: CancellationToken) => {
      ...
      context.subscriptions.push(
        commands.registerCommand(
          ...
          async (classFileUri: Uri): Promise<boolean> => {
            const resolveRequest: SourceAttachmentRequest = {
              classFileUri: classFileUri.toString(),
            };
            const resolveResult: SourceAttachmentResult = await (<SourceAttachmentResult>(
              commands.executeCommand(
                Commands.EXECUTE_WORKSPACE_COMMAND,
            ...
            if (sourceFileUris && sourceFileUris.length) {
              ...
              const updateResult: SourceAttachmentResult = await (<SourceAttachmentResult>(
                commands.executeCommand(
                  Commands.EXECUTE_WORKSPACE_COMMAND,
                  Commands.UPDATE_SOURCE_ATTACHMENT,
      ...
      if (extensions.onDidChange) {
        // Theia doesn't support this API yet
      ...
      context.subscriptions.push(
        commands.registerCommand(
          Commands.LEARN_MORE_ABOUT_REFACTORING,
          async (kind: CodeActionKind) => {
            const sectionId: string = javaRefactorKinds.get(kind) || '';
            markdownPreviewProvider.show(
              context.asAbsolutePath(
                path.join('document', `${Commands.LEARN_MORE_ABOUT_REFACTORING}.md`),
              ),
              'Java Refactoring',
              sectionId,
              context,
      ...
      languages.registerCodeActionsProvider(
        ...
        new PomCodeActionProvider(context),
        pomCodeActionMetadata,
      );
    });
  }
...
async function showImportFinishNotification(context: ExtensionContext) {
  const neverShow: boolean | undefined = context.globalState.get<boolean>(
  ...
  if (!neverShow) {
    ...
    if (
      projectUris.length === 0 ||
      (projectUris.length === 1 && projectUris[0].includes('jdt.ls-java-project'))
    ) {
      return;
    }
    ...
    window
      .showInformationMessage('Projects are imported into workspace.', ...options)
      .then((choice) => {
...
function setProjectConfigurationUpdate(
