...
function createDocumentSymbolProvider(): DocumentSymbolProvider {
    return <DocumentSymbolProvider>{
        provideDocumentSymbols: async (
            ...
            const symbolResponse = await languageClient.sendRequest(
            ...
            if ((<any>symbolResponse[0]).containerName) {
                return languageClient.protocol2CodeConverter.asSymbolInformations(
                    <clientSymbolInformation[]>symbolResponse,
                );
            }

            return languageClient.protocol2CodeConverter.asDocumentSymbols(
                <clientDocumentSymbol[]>symbolResponse,
            );
        },
    };
}
